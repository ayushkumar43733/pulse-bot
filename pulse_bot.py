...
def get_latest_upload():
    """
    Returns (video_id, title, thumbnail_url) for the most recent upload
    on the channel's uploads playlist, or (None, None, None) on error.
...
async def check_youtube_live_status():
    ...
    if live_now and not was_youtube_live:
        ...
        print("[INFO] Klurge's YouTube stream ended.")
...
async def on_ready():
    ...
    # Set initial status (reflects live state if already streaming)
    if kick_live_now:
        await client.change_presence(
            status=discord.Status.online,
            activity=discord.Streaming(name=f"{kick_title}", url=f"https://kick.com/{KICK_SLUG}"),
        )
    else:
        await client.change_presence(
            status=discord.Status.online,
